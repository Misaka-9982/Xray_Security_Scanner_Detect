        ...
        for path, im, im0s, vid_cap, s in dataset:  # 图片读取使用cv2.imread()，在dataset对象的类中，迭代器函数__next__()中调用
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    # Write results
                    for *xyxy, conf, cls in reversed(det):  # xyxy, 框坐标长度；conf，置信度；cls，识别种类names中的序号
...
def parse_opt():
    ...
    opt = parser.parse_args()  # 解析器解析出的参数封装在opt中，opt.参数即可访问
    opt.imgsz *= 2 if len(opt.imgsz) == 1 else 1  # 只给长/宽时就变为长*宽，两个都给时不作变化
    print_args(vars(opt))  # vars()返回对象属性和值的字典
    return opt
